...
import { Plus, Receipt, TrendingUp, TrendingDown, DollarSign, BarChart3, Grid3X3 } from 'lucide-react';
...
export default function UtilityBillsPage() {
  ...
  const fetchBills = async () => {
    ...
    const sortedBills = data.sort((a: UtilityBill, b: UtilityBill) => a.month.localeCompare(b.month));
    setBills(sortedBills);

    // Auto-select all utilities when data loads
    if (sortedBills.length > 0 && selectedUtilities.length === 0) {
      const allUtilityNames = new Set<string>();
      sortedBills.forEach((bill: UtilityBill) => {
        Object.keys(bill.utilities).forEach(name => allUtilityNames.add(name));
      });
      setSelectedUtilities(Array.from(allUtilityNames));
    }
  };
  ...
  const getChartColor = (utilityName: string) => {
    const name = utilityName.toLowerCase();
    if (name.includes('electric') || name.includes('electricity')) return '#ca8a04';
    if (name.includes('gas')) return '#ea580c';
    if (name.includes('water')) return '#2563eb';
    if (name.includes('internet') || name.includes('wifi')) return '#9333ea';
    if (name.includes('trash') || name.includes('garbage')) return '#6b7280';
    if (name.includes('sewer')) return '#0d9488';
    if (name.includes('phone')) return '#4f46e5';
    return '#64748b';
  };
  ...
  const chartData = bills.map(bill => {
    const dataPoint: any = {
      month: new Date(bill.month + '-01').toLocaleDateString('en-US', { month: 'short', year: '2-digit' }),
      total: bill.total,
    };

    // Add each utility as a separate field
    Object.entries(bill.utilities).forEach(([name, value]) => {
  ...
  const toggleUtility = (utilityName: string) => {
    setSelectedUtilities(prev => {
      if (prev.includes(utilityName)) {
        return prev.filter(u => u !== utilityName);
      } else {
        return [...prev, utilityName];
      }
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-slate-50 to-zinc-50 dark:from-gray-950 dark:via-slate-950 dark:to-zinc-950">
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 py-6 sm:py-8 lg:py-12 max-w-7xl">
        ...
        {/* View Toggle & Add Bill Button */}
        <div className="mb-4 sm:mb-6 flex flex-wrap gap-3 items-center justify-between">
          <div className="flex gap-2">
            <Button
              onClick={() => setViewMode('grid')}
              variant={viewMode === 'grid' ? 'default' : 'outline'}
              className={viewMode === 'grid' ? 'bg-gradient-to-r from-slate-700 to-gray-800 hover:from-slate-800 hover:to-gray-900' : ''}
            >
            ...
            <Button
              ...
              className={viewMode === 'chart' ? 'bg-gradient-to-r from-slate-700 to-gray-800 hover:from-slate-800 hover:to-gray-900' : ''}
            >
        ...
        {/* Chart View */}
        {viewMode === 'chart' && bills.length > 0 && (
          <div className="space-y-4 sm:space-y-6 mb-6">
            {/* Utility Filter Chips */}
            <Card>
              <CardContent className="pt-4 sm:pt-6">
                <h3 className="font-semibold text-lg mb-3">Filter Utilities</h3>
                <div className="flex flex-wrap gap-2">
                  {Array.from(allUtilityNames).map(utilityName => {
                    const isSelected = selectedUtilities.includes(utilityName);
                    return (
                      <Button
                        key={utilityName}
                        onClick={() => toggleUtility(utilityName)}
                        variant={isSelected ? 'default' : 'outline'}
                        size="sm"
                        className={isSelected ? 'bg-gradient-to-r from-slate-700 to-gray-800 hover:from-slate-800 hover:to-gray-900' : ''}
                      >
                        <span className="mr-2">{getUtilityIcon(utilityName)}</span>
                        {utilityName}
                      </Button>
                    );
                  })}
            ...
            <Card>
              <CardContent className="pt-4 sm:pt-6">
                <h3 className="font-semibold text-lg mb-4">Utility Cost Trends</h3>
                <div className="w-full h-[400px] sm:h-[500px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      />
                      <Legend />
                      {selectedUtilities.map(utilityName => (
                        <Bar
                          key={utilityName}
                          dataKey={utilityName}
                          fill={getChartColor(utilityName)}
                          radius={[0, 0, 0, 0]}
                        />
                      ))}
                    </BarChart>
